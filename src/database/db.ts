...
class DatabaseEngine {
  ...
  private runMigrations() {
    const versionStr = localStorage.getItem(DB_VERSION_KEY);
    const version = versionStr ? parseInt(versionStr, 10) : CURRENT_SCHEMA_VERSION;
    if (version < CURRENT_SCHEMA_VERSION) {
      // Future migration steps here
      localStorage.setItem(DB_VERSION_KEY, String(CURRENT_SCHEMA_VERSION));
    }
  }
  ...
  private persist() {
    try {
      ...
      console.error('Failed to persist database to local storage:', e);
    }
  }
